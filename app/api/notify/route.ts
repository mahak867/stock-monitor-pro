...
export async function POST(req: NextRequest) {
  try {
    ...
    const safeSymbol       = htmlEscape(symbol);
    const safeCondition    = htmlEscape(condition);
    const safeTriggerPrice = htmlEscape(Number(triggerPrice).toFixed(2));
    const safeCurrentPrice = htmlEscape(Number(currentPrice).toFixed(2));
    const direction        = safeCondition === 'above' ? '▲ above' : '▼ below';

    const { error } = await resend.emails.send({
      from: 'StockPro Alerts <alerts@stockpro.app>',
      to: [email],
      subject: `🔔 Price Alert: ${safeSymbol} is ${safeCondition} $${safeTriggerPrice}`,
      html: `
        <div style="font-family:system-ui,sans-serif;max-width:480px;margin:0 auto;padding:24px;background:#0b0d1c;color:#f1f5f9;border-radius:12px;">
          ...
          <div style="background:#06081a;border:1px solid rgba(99,102,241,0.2);border-radius:8px;padding:16px;margin-bottom:16px;">
            <div style="font-size:24px;font-weight:700;margin-bottom:4px;">${safeSymbol}</div>
            <div style="font-size:14px;color:#94a3b8;">
              Current price: <strong style="color:#f1f5f9;">$${safeCurrentPrice}</strong>
              has gone ${direction} your alert price of
              <strong style="color:#818cf8;">$${safeTriggerPrice}</strong>
            </div>
          </div>
